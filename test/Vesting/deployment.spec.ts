...
describe('Vesting - Deployment', function () {
  it('Grants DEFAULT_ADMIN_ROLE and VESTING_ADMIN_ROLE to owner', async function () {
    const {
      vesting,
      owner
    } = await loadFixture(deployVestingContract)

    const defaultAdmins = await vesting.getRoleMembers(await vesting.DEFAULT_ADMIN_ROLE())
    const vestingAdmins = await vesting.getRoleMembers(await vesting.VESTING_ADMIN_ROLE())

    expect(defaultAdmins).to.deep.equal([ owner.address ])
    expect(vestingAdmins).to.deep.equal([ owner.address ])
  })
